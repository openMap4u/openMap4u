...
class Canvas implements Plugable, DrawOrWriteable, SetAreaOfInterestOrDrawOrWriteable,
    ...
    public final SetAreaOfInterest worldUnits(Length worldUnits) {
        this.mWorldUnits = worldUnits;
        return this;
    }
    ...
    @Override
    public SetAreaOfInterestOrDrawOrWriteable center(double centerX, double centerY) {
        ...
        if (builder.getStyle().isVisible()) {
            ...
            if (builder.isPoint()) {
                for (Object point : builder.getPoints()) {
                    try {
                        previousDrawnShape = draw(
                                new TransformUtil()
                                .transform((Point<?, ?>) point,
                                        previousDrawnShape,
                                        this.mOutputFormat
                                        .getGlobalTransform()),
                                builder, previousDrawnShape);
                    } catch (Exception ex) {
                        Logger.getLogger(Canvas.class.getName()).log(
