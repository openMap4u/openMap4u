...
public class AreaChartTest extends AbstractOpenMap4uTest {
    ...
    public void testVertcialAreaChart() throws IOException {
       ...
       /* draw vertical lines */
        ...
        /* label y axis */
        MockupData.get().getValues(1, 4, 4).stream().forEach(value -> canvas.draw(oM4u.get(Text.class).color(Color.DARK_GRAY).center(0, value).align(RIGHT,MIDDLE).offset(-.5, 0).size(2.5).text("%2.0f",value)));

        /* add symbols */
        data.stream().forEach(value -> canvas.draw(oM4u.get(Rectangle.class).color(Color.RED).fill(Color.WHITE).center(value.getKey(), value.getValue()[0]).width(0.25)));
