...
public class LineChartTest extends AbstractOpenMap4uTest {
    ...
    public void testSimpleCircleRaster() throws IOException {
        ...
        Stream.of(1, 2, 3).map(value -> canvas.draw(oM4u.get(Circle.class).center(5, 4).radius(value).size((value % 2 == 0) ? 0.5 : 1).color((value % 2 == 0) ? Color.GRAY
                : Color.BLACK)));
        /* draw the vertical raster lines unsing*/

        Stream.of(0d, 45d, 90d, 135d).map(value -> oM4u.get(LineChart.class).center(5, 4).from(-3.5, 0).to(3.5, 0).size((value % 2 == 0) ? 1 : 0.5).color((value % 2 == 0) ? Color.BLACK
    ...
    public void testCircleRasterWithHighightedRowsAndColumns() throws IOException {
        ...
        DrawOrWriteable canvas = oM4u.getCanvas(10, 8);
        /*  draw the horizontal raster lines */
        Stream.of(1, 2, 3).map(value -> canvas.draw(oM4u.get(Circle.class).center(5, 4).color(Color.GRAY).radius(value)));
        /* draw the vertical raster lines */
        Stream.of(0d, 45d, 90d, 135d).map(value -> oM4u.get(LineChart.class).center(5, 4).from(-3.5, 0).to(3.5, 0).color(Color.GRAY).rotate(value)).forEach(e -> canvas.draw(e));
        /* persist your result */
    ...
    public void test() throws IOException {
        ...
        DrawOrWriteable draw = oM4u.getCanvas(10, 8);
        /*  draw the horizontal raster lines */
        Stream.of(1, 2, 3).map(value -> oM4u.get(Circle.class).center(5, 4).color(Color.GRAY).radius(value)).forEach(primitive -> draw.draw(primitive));
        /* draw the vertical raster lines */
        Stream.of(0d, 45d, 90d, 135d).map(value -> oM4u.get(LineChart.class).center(5, 4).from(-3.5, 0).to(3.5, 0).color(Color.GRAY).rotate(value)).forEach(primitive -> draw.draw(primitive));
        /* persist your result */
