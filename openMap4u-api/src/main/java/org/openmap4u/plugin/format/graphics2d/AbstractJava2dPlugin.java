...
import static org.openmap4u.common.Globals.DEFAULT_RESSOURCE_BUNDLE;
...
abstract class AbstractJava2dPlugin implements Outputable {
	 ...
	 * The logger.
	 */
	private static final Logger LOGGER = Logger.getLogger(AbstractJava2dPlugin.class.getName(),
			DEFAULT_RESSOURCE_BUNDLE);
	...
	final AffineTransform getTransform(Point2D point, DrawableTransformable individual, Shape shape) {
		...
		return this.mTransformUtil.transform(global, point, 1 / global.getScaleX() * this.mDrawingUnit2PixelFactor,
				-1 / global.getScaleY() * this.mDrawingUnit2PixelFactor, individual, shape);
	}

	public Shape draw(Point2D point, ShapeDrawable shape, Shape shapeBounds) {
		/* set the alphaValue */
		this.mG2D
				.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, (float) shape.getStyle().getAlpha()));
		/* set the stroke size */
		this.mG2D.setStroke(new BasicStroke((float) (shape.getStyle().getStrokeSize() * this.mStrokeUnit2PixelFactor),
	...
	public Shape drawImage(Point2D point, ImageDrawable image) {
		...
		try {
			...
			/* set the alphaValue */
			this.mG2D.setComposite(
					AlphaComposite.getInstance(AlphaComposite.SRC_OVER, (float) image.getStyle().getAlpha()));
			/*
			 * take the multiplication factor into acount, as well as the
			 * coordinate system
			 */
			AffineTransform transform = getTransform(point, image.getTransform(), new Rectangle2D.Double(0, 0,
					img.getWidth() / this.mDrawingUnit2PixelFactor, img.getHeight() / this.mDrawingUnit2PixelFactor));
			transform.concatenate(new AffineTransform(1 / this.mDrawingUnit2PixelFactor, 0, 0,
					-1 / this.mDrawingUnit2PixelFactor, 0, img.getHeight() / this.mDrawingUnit2PixelFactor));
			this.mG2D.drawImage(img, transform, null);
			imageOutline = transform
					.createTransformedShape(new Rectangle2D.Double(0, 0, img.getWidth(), img.getHeight()));
		} catch (Exception e) {
	...
	public Shape drawText(Point2D point, TextDrawable text) {
		...
		GlyphVector glyphVector = font.createGlyphVector(this.mFontRenderContext, text.getText());
		...
		Polygon shape2Draw = new Polygon().fill(text.getStyle().getFontColor()).color(null)
				.transparence(text.getStyle().getAlpha()).shape(fontPath);
		...
		Shape outline = new Rectangle2D.Double(0, 0, fontPath.getBounds2D().getWidth(),
				text.getStyle().getFontSize() * mStrokeUnits2DrawingUnits);
	...
	Font getFont(TextStyleable style) {
		...
		return new Font(fontFamily, fontStyle,
				(int) (style.getFontSize() * this.mStrokeUnits2DrawingUnits * 100 * DPI / DOT + ZERO_POINT_5))
						.deriveFont(mFontTransformation);
	}

	@Override
	public final void setUp(Shape shape, final Length worldUnits, final Length drawingUnits, final Length strokeUnits,
		...
		this.mBufferedImage = new BufferedImage(canvasDimension.width, canvasDimension.height,
				BufferedImage.TYPE_4BYTE_ABGR);
		/* generate the global transformation */
		/* first change the orientation of the coordinate system */
		globalTransform.preConcatenate(new AffineTransform(mWorldUnit2PixelFactor, 0, 0, -mWorldUnit2PixelFactor, 0,
		...
		this.mFontRenderContext = new FontRenderContext(this.mGlobalTransform, true, true);
	}
	...
	final Dimension getCanvasSize(Shape canvasShape, Length drawingUnits) {
		return new Dimension((int) drawingUnits.convert(canvasShape.getBounds2D().getWidth(), Length.PIXEL),
				(int) drawingUnits.convert(canvasShape.getBounds2D().getHeight(), Length.PIXEL));
	}
	...
	public void before() {
		...
		this.mG2D.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
	}
