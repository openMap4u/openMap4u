...
public class LineTest extends AbstractOpenMap4uTest {
    ...
    public void testDrawSimpleVerticalLinesWithLabel() throws IOException {
        ...
        /* draw the line */
        getData().forEach(value -> canvas.draw(oM4u.get(Text.class).text(value).point(value, 1).size(3.5).align(CENTER, TOP).offset(0, 2)));
    ...
    public void testSimpleHorizontalTicWithLabel() throws IOException {
        ...
        values.stream().forEach(value -> draw.draw(oM4u.get(Line.Horizontal.class).length(0.5).color(Color.BLACK).point(3, value)).draw(oM4u.get(Text.class).point(LEFT, MIDDLE).align(RIGHT, MIDDLE).text("%1$2.1f", value)));
        /* persist your result */
        draw.write(getPackagePath("simpleHorizontalTic.png"));
    ...
    public void testVerticalLineLength() throws IOException {
        ...
        canvas.draw(oM4u.get(Text.class).point(1, .5).align(CENTER, BOTTOM).size(2.5).offsetX(-.2).rotate(90).text("length"));
        canvas.draw(oM4u.get(Text.class).point(1, .5).align(LEFT, MIDDLE).size(2.5).offsetX(.2).text("point"));
    ...
    public void testHorizontalLineLength() throws IOException {
        ...
        canvas.draw(oM4u.get(Text.class).point(1, .5).align(CENTER, BOTTOM).size(2.5).offsetY(.2).text("length"));
        canvas.draw(oM4u.get(Text.class).point(1, .5).align(CENTER, TOP).size(2.5).offsetY(-.2).text("point"));
    ...
    public void testHorizontalLine() throws IOException {
        ...
        canvas.draw(oM4u.get(Text.class).point(.6, .5).align(CENTER, BOTTOM).size(2.5).offsetY(.2).text("fromX"));
        canvas.draw(oM4u.get(Text.class).point(1.4, .5).align(CENTER, BOTTOM).size(2.5).offsetY(.2).text("toX"));
        canvas.draw(oM4u.get(Text.class).point(1.4, .5).align(LEFT, MIDDLE).size(2.5).offsetX(.2).text("y"));
    ...
    public void testVerticalLine() throws IOException {
        ...
        canvas.draw(oM4u.get(Text.class).point(1, .1).align(RIGHT, BOTTOM).size(2.5).offsetX(-.2).text("x"));
        canvas.draw(oM4u.get(Text.class).point(1, .9).align(LEFT, TOP).size(2.5).offsetX(.2).text("toY"));
        canvas.draw(oM4u.get(Text.class).point(1, .1).align(LEFT, BOTTOM).size(2.5).offsetX(.2).text("fromY"));
