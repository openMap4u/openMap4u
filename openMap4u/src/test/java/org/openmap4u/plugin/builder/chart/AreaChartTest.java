...
public class AreaChartTest extends AbstractOpenMap4uTest {
    ...
    public void testVertcialAreaChart() throws IOException {
       ...
       /* draw vertical lines */
        ...
        /* label x axis */
        data.stream().forEach(value -> canvas.draw(oM4u.get(Text.class).color(Color.BLACK).point(value.getKey(), 0).align(CenterTop).offset(0, -.25).size(2.5).text(value.getKey())));
        /* label y axis */
        MockupData.get().getValues(1, 4, 4).stream().forEach(value -> canvas.draw(oM4u.get(Text.class).color(Color.DARK_GRAY).point(0, value).align(RightMiddle).offset(-.5, 0).size(2.5).text("%2.0f",value)));
        ...
        data.stream().forEach(value -> canvas.draw(oM4u.get(Text.class).color(Color.BLACK).point(value.getKey(), value.getValue()[0]).offset(0, 0.35).rotate(75).size(2.5).align(LeftBottom).text("%2.2f", value.getValue()[0])));
