...
public class PolylineTest extends AbstractOpenMap4uTest {
    ...
    public void drawBezier(Point2D.Double from, Point2D.Double cp1, Point2D.Double cp2, Point2D.Double to, String outputFile) throws IOException {
        getCanvas(1.4)
                ...
                .draw(getDefaultOpenMap4u().getBuilder(Line.class).from(to.x, to.y).to(cp2.x, cp2.y).color(Color.GRAY)).draw(getDefaultOpenMap4u().getBuilder(Polyline.class).moveTo(from.x, from.y).bezierTo(cp1.x, cp1.y, cp2.x, cp2.y, to.x, to.y).color(Color.BLACK).size(.5))
    ...
    public void testQuadTo() throws IOException {
        ...
        getCanvas(1).draw(getDefaultOpenMap4u().getBuilder(Line.class).from(from.x, from.y).to(cp.x, cp.y).color(Color.GRAY))
                .draw(getDefaultOpenMap4u().getBuilder(Line.class).from(to.x, to.y).to(cp.x, cp.y).color(Color.GRAY)).draw(getDefaultOpenMap4u().getBuilder(Polyline.class).moveTo(from.x, from.y).quadTo(cp.x, cp.y, to.x, to.y).color(Color.BLACK).size(.5))
    ...
    public void testLineTo() throws IOException {
        ...
        Point2D.Double to = new Point2D.Double(0.75, 0.75);
         getCanvas(1)    .draw(getDefaultOpenMap4u().getBuilder(Line.class).from(from.x, from.y).to(to.x, to.y).color(Color.BLACK).size(.5))
