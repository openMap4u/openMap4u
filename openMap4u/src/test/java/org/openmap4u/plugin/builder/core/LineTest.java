...
public class LineTest extends AbstractOpenMap4uTest {
    ...
    public void testDrawSimpleVerticalLines() throws IOException {
        ...
        Vertical line = oM4u.create(Vertical.class).line(0, 1, 7).size(1).color(Color.GRAY);
        /* draw the line */
    ...
    public void testSimpleHorizontalTic() throws IOException {
        ...
        /* 3. draw your primitive(s). */
        Line.Horizontal tic = oM4u.create(Line.Horizontal.class).length(0.5).color(Color.BLACK);
    ...
    public void testVerticalHorizontalTic() throws IOException {
        ...
        /* 3. draw your primitive(s). */
        Line.Vertical tic = oM4u.create(Line.Vertical.class).length(0.5).color(Color.BLACK);
    ...
    public void testVerticalLineLength() throws IOException {
        ...
        canvas.draw(oM4u.create(Vertical.class).length(.8).point(1, .5));
        canvas.draw(oM4u.create(Text.class).point(1, .5).align(Position.CenterBottom).size(2.5).offsetX(-.2).rotate(90).text("length"));
        canvas.draw(oM4u.create(Text.class).point(1, .5).align(Position.LeftMiddle).size(2.5).offsetX(.2).text("point"));
    ...
    public void testHorizontalLineLength() throws IOException {
        ...
        /* 2. get an canvas and specify the size which you want to draw */
        DrawOrWriteable canvas = oM4u.getCanvas(2, 1);
        canvas.draw(oM4u.create(Horizontal.class).length(.8).point(1, .5));
        canvas.draw(oM4u.create(Text.class).point(1, .5).align(Position.CenterBottom).size(2.5).offsetY(.2).text("length"));
        canvas.draw(oM4u.create(Text.class).point(1, .5).align(Position.CenterTop).size(2.5).offsetY(-.2).text("point"));
    ...
    public void testHorizontalLine() throws IOException {
        ...
        /* 2. get an canvas and specify the size which you want to draw */
        DrawOrWriteable canvas = oM4u.getCanvas(2, 1);
        canvas.draw(oM4u.create(Horizontal.class).line(-.4, .4, .0).point(1, .5));
        canvas.draw(oM4u.create(Text.class).point(.6, .5).align(Position.CenterBottom).size(2.5).offsetY(.2).text("fromX"));
        canvas.draw(oM4u.create(Text.class).point(1.4, .5).align(Position.CenterBottom).size(2.5).offsetY(.2).text("toX"));
        canvas.draw(oM4u.create(Text.class).point(1.4, .5).align(Position.LeftMiddle).size(2.5).offsetX(.2).text("y"));
    ...
    public void testVerticalLine() throws IOException {
        ...
        canvas.draw(oM4u.create(Vertical.class).line(0, -.4, .4).point(1, .5));
        canvas.draw(oM4u.create(Text.class).point(1, .1).align(Position.RightBottom).size(2.5).offsetX(-.2).text("x"));
        canvas.draw(oM4u.create(Text.class).point(1, .9).align(Position.LeftTop).size(2.5).offsetX(.2).text("toY"));
        canvas.draw(oM4u.create(Text.class).point(1, .1).align(Position.LeftBottom).size(2.5).offsetX(.2).text("fromY"));
